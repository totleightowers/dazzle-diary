...
public class MainActivity extends Activity {
    ...
    public class Bridge {
        ...
        /** Open the Diamond Art Club screen: sign in, then — from that one page —
         *  read colours, tick kits through DAC's API, read colours again. The
         *  result comes back to window.__dacSyncDone. */
        @JavascriptInterface
        public boolean dacSync(String kits, String tick, String legends) {
            mustBeOurPage();
            if (kits == null || tick == null || legends == null) return false;
            if (kits.length() + tick.length() + legends.length() > 512 * 1024) return false;
            final Intent go = new Intent(MainActivity.this, DacActivity.class)
                .putExtra(DacActivity.EXTRA_KITS, kits)
                .putExtra(DacActivity.EXTRA_TICK, tick)
